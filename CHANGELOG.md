...
- **打包版第二次開啟就整片錯誤** — 關閉 app 後背景服務會留下來佔著連接埠，下次開啟時新的服務起不來、畫面上每個請求都失敗。現在關視窗、Cmd+Q、系統列 Quit 都會正常結束背景服務；app 當掉或被強制結束時，背景服務也會在幾秒內自己結束。從 1.0.0 升級時，舊版留下的背景服務仍會佔著連接埠：升級後若還是整片錯誤，重開機一次，或在活動監視器（Windows 為工作管理員）結束 `starscope-sidecar`
