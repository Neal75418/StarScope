...
- **自上次以來摘要** — Dashboard 最上方改為「自上次看過之後」的摘要，取代原本的「需要注意」列：breaking／security release、中高嚴重度訊號、HN ≥ 50 分、觸發的警報列為重點，一般 release 收在「其他更新」；有重點時開啟 app 直接落在 Dashboard

### 修復

- **系統列出現兩個圖示** — 除了 StarScope 的圖示，旁邊還有一塊沒有選單的白色方塊；現在只剩一個
- **連接埠被其他程式佔用時整片錯誤、重複開啟出現兩個 app** — 連接埠 8008 被舊版背景服務或其他程式佔著時，現在直接說明是誰佔著、該怎麼處理；上一個 app 剛關閉時會等它讓出連接埠。再次開啟 StarScope 會叫出已開著的視窗，不再開第二個
- **打包版開啟後顯示「資料引擎未執行」、頁面空白** — 背景服務啟動要十幾秒，前端在它起來前就放棄連線，要換頁或等一分鐘才有資料。現在啟動期間顯示「資料引擎啟動中…」，服務一起來資料就自己出現；一直起不來時直接說明，不會誤顯示成「還沒追蹤任何專案」
- **打包版第二次開啟就整片錯誤** — 關閉 app 後背景服務會留下來佔著連接埠，下次開啟時新的服務起不來、畫面上每個請求都失敗。現在關視窗、Cmd+Q、系統列 Quit 都會正常結束背景服務；app 當掉或被強制結束時，背景服務也會在幾秒內自己結束。從 1.0.0 升級時，舊版留下的背景服務可能還佔著連接埠，畫面會說明「另一個 StarScope 背景服務佔用了連接埠 8008」：重開機一次，或在活動監視器（Windows 為工作管理員）結束 `starscope-sidecar`
