...
- `useFeed()` 回傳 `{ items, feedDate, isLoading, isGenerating, feedback, refresh }`
  - **自動產生機制**：當日 feed 為空時自動觸發一次 `generateFeed`，由 `triggerGenerate` 這個**單一入口**統一設定 `autoGenerated` ref。effect 自動觸發與手動 `refresh()` 都必須走這個入口，否則兩條路徑會競態、同一次操作打兩次 API
...
- 動態行高：`rowHeight` 支援 `(index: number) => number`
  - 收合 `COLLAPSED_ITEM_SIZE = 296px`（卡片 280 + 間距 16）
  - 展開 `EXPANDED_ITEM_SIZE = 596px`（加圖表 300px）
