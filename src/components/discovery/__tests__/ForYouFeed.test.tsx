...
describe("ForYouFeed", () => {
  ...
  it("track button calls onAddToWatchlist and sends starred feedback", async () => {
    ...
    renderWithClient(<ForYouFeed onAddToWatchlist={onAdd} watchlistFullNames={new Set()} />);
    await screen.findByText("a/one");
  ...
  it("locks the track button for repos already in the watchlist", async () => {
    // 只鎖「請求進行中」不夠：加成功之後卡片還在清單上，再按一次會拿到
    // 400「已在追蹤清單」，使用者看到的是泛用錯誤 toast。
    const onAdd = vi.fn();
    renderWithClient(
      <ForYouFeed onAddToWatchlist={onAdd} watchlistFullNames={new Set(["a/one"])} />
    );
    await screen.findByText("a/one");
