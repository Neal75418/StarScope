...
describe("useFeed", () => {
  ...
  it("manual refresh does not cause a duplicate generate when result stays empty", async () => {
    // 回歸測試：情境為「今日 feed 一開始就是空的」，此時 auto-generate effect 與
    // 使用者手動 refresh() 幾乎同時搶跑。修正前 autoGenerated.current 只在 effect
    // 分支內設置，refresh() 繞過它，導致同一輪最終打了兩次 generateFeed。
    vi.mocked(apiClient.getFeed).mockResolvedValue({ feed_date: "2026-08-01", items: [] });
    vi.mocked(apiClient.generateFeed).mockResolvedValue({
      feed_date: "2026-08-01",
      generated: 0,
    ...
    const { result } = renderHook(() => useFeed(), { wrapper: createWrapper() });
    // 資料尚未載入完成（isEmpty 尚未成立）時就手動觸發 refresh，
    // 讓它與稍後 query 落地為空陣列時的 auto-generate effect 互相搶跑。
    result.current.refresh();

    await waitFor(() => expect(apiClient.generateFeed).toHaveBeenCalledTimes(1));
    // 再等一輪，確認 auto-generate effect 沒有因為 ref 沒被 refresh() 標記而補打第二次。
    await new Promise((r) => setTimeout(r, 50));
    expect(apiClient.generateFeed).toHaveBeenCalledTimes(1);
  });
